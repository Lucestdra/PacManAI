...
# Function to preprocess the game frame
...
class Agent():
    ...
    # Store experiences and trigger learning if memory size exceeds the minimum batch size
    def step(self, state, action, reward, next_state, done):
        state = preprocess_frame(state)
        next_state = preprocess_frame(next_state)
        self.memory.append((state, action, reward, next_state, done))
        if len(self.memory) > min_batch_size:
            experiences = random.sample(self.memory, k=min_batch_size)
...
# Function to show a video of the trained model
...
# Show video of the trained model
